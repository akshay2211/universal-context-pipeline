...
| Area | Detail |
|---|---|
| Crate shape | Single crate `ucp-local`, headless binary + lib. No workspace, no Tauri. |
...
| CLI | `ucp-local index <path>`, `ucp-local serve`, `ucp-local watch <path>`, `ucp-local status`. |
...
### Success criteria
1. `ucp-local index ~/Documents/notes` indexes 1000 markdown files under 60s on M-series Mac.
2. Claude Desktop pointed at `ucp-local serve` can call `search_local_context("foo")` and get grounded chunks back with citations.
3. Editing a file triggers re-index of just that file within ~2s.
4. End-to-end install: `cargo install ucp-local` + `ollama pull nomic-embed-text` + Claude Desktop config edit.
...
- **Week 3** — `notify` watcher + conversation-export ingester + `ucp-local ask` + README + release artifacts.
...
- **`ucp-local ask <question>` CLI.** Local Ollama chat model + search tool. Terminal Q&A without needing any MCP client. Huge first-run experience win.
